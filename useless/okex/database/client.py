...
from funcoin.database.base import Base, BaseTable, create_all, create_session, session
...
class OkexClientAccountBalance(Base, BaseTable):
    ...
    def delete_dated():
        try:
            ...
            # 先查询，后删除
            session.query(OkexClientAccountBalance).filter(
                OkexClientAccountBalance.updateTime < update_time
            ).delete()
            session.commit()
            session.query(OkexClientAccountBalance).filter(
                OkexClientAccountBalance.eqUsd < 1
            ).delete()
            session.commit()
