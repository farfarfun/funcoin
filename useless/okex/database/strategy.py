...
from funcoin.database.base import Base, BaseTable
from funcoin.strategy.utils import trade_api
...
class OkexStrategyAutoSeller(Base, BaseTable):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.max_worth = (
            kwargs.get("max_worth") or kwargs.get("init_worth") or kwargs.get("worth")
        )
        self.min_worth = (
            kwargs.get("min_worth") or kwargs.get("init_worth") or kwargs.get("worth")
        )
    ...
    def to_json(self):
        return {"coin_id": self.coin_id, "usdt": round(self.worth, 2)}

    def buy(self, worth=50):
        return trade_api.place_order(
            instId=self.coin_id, tdMode="cash", side="buy", ordType="market", sz=worth
        )
    ...
    def instance(coin_id, price=None, count=None, worth=None, init_worth=50):
        return OkexStrategyAutoSeller(
            coin_id=coin_id,
            price=price,
            count=count,
            worth=worth,
            init_worth=init_worth,
        )
