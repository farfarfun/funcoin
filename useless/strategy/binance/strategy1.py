...
from funcoin.task import AccountTask, BaseTask, MarketTask, Ticker24HTask
...
class StrategyTask(BaseTask):
    ...
    def buy_auto(self):
        account = pd.read_sql(
            sql=f"select * from {AccountTask.table_name}", con=self.engine.connect()
        )
        curr = 0
        ...
        if curr < 12:
            ...
            return
        market = pd.read_sql(
            sql=f"select * from {MarketTask.table_name}", con=self.engine.connect()
        )
        ticker24h = pd.read_sql(
            sql=f"select * from {Ticker24HTask.table_name}", con=self.engine.connect()
        )
        ticker24h = ticker24h.sort_values(["quoteVolume"], ascending=False).reset_index(
            drop=True
        )
    ...
    def sell_auto(self):
        data = pd.read_sql(
            f"select * from {self.table.table_name}", con=self.engine.connect()
        )
        for row in json.loads(data.to_json(orient="records")):
            try:
                ...
                if (
                    time.time() * 1000 - timestamp > 30 * 60 * 1000
                    and abs((buy_price - price) / price) > 0
                ):
