...
class Strategy:
    ...
    def refresh_markets(self):
        ...
        df.to_sql(
            name=self.table_markets, con=self.engine.connect(), if_exists="replace"
        )
    ...
    def refresh_account(self):
        ...
        self.account.to_sql(
            name=self.table_account, con=self.engine.connect(), if_exists="replace"
        )

    def update_ohlcv_once(
        self, since, symbol, timeframe="1m", limit=500, *args, **kwargs
    ):
        ...
        df["symbol"] = symbol
        df.to_sql(
            f"{self.table_ohlcv}_{timeframe}",
        ...
        if timeframe == "1m":
            for _ in tqdm(range(4 * 30 * 3)):
                ...
                for symbol in symbols:
                    self.update_ohlcv_once(
                        int(t.timestamp() * 1000), symbol, timeframe, limit=600
                    )
            ...
            for symbol in tqdm(symbols):
                self.update_ohlcv_once(
                    int(t.timestamp() * 1000), symbol, timeframe, limit=500
                )
        else:
    ...
    def save_ohlcv_to_csv(
        self, filename="data", timeframe="1m", page_size=500000, total_step=1000
    ):
