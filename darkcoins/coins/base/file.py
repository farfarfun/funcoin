...
from darkcoins.base.drive.lanzou import LanzouDirectory
from darkcoins.coins.base.load import LoadDataKline, LoadTradeKline
...
class FileProperty:
    ...
    def file_path_dir(self, absolute=True):
        path = f"darkcoins/{self.exchange_name}/{self.data_type}-{self.freq}-{self.timeframe}"
        if absolute:
...
class DataFileProperty:
    ...
    def sync(self, path):
        ...
        self.drive.sync(f'{path}/darkcoins')
