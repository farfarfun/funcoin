...
from darkcoin.base.drive.lanzou import LanzouDirectory
from darkcoin.coins.base.load import LoadDataKline, LoadTradeKline
...
class FileProperty:
    ...
    def file_path_dir(self, absolute=True):
        path = f"darkcoin/{self.exchange_name}/{self.data_type}-{self.freq}-{self.timeframe}"
        if absolute:
...
class DataFileProperty:
    ...
    def sync(self, path):
        ...
        self.drive.sync(f'{path}/darkcoin')
