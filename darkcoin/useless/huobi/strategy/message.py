...
class HuoBiMessage:
    def __init__(self, name='huobi', webhook=None, agent_id=None, secret=None, company_id=None):
        self.name = name
        # 机器人webhook
        self.webhook = read_secret("wechat", "darkchats", "huobi", name, "webhook", value=webhook)
        # 应用ID
        self.agent_id = read_secret("wechat", "darkchats", "huobi", name, "agent_id", value=agent_id)
        # 企业ID
        self.company_id = read_secret("wechat", "darkchats", "huobi", name, "company_id", value=company_id)
        # 应用Secret
        self.secret = read_secret("wechat", "darkchats", "huobi", name, "secret", value=secret)
    ...
    def check_token(self):
        # 通行密钥
        access_token = None
        access_token = read_secret("wechat", "darkchats", "huobi", self.name, "access_token")
        if not access_token:
